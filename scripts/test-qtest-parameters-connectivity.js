...
/**
 * Script to test connectivity with qTest Parameters API
 * 
 * This script verifies connectivity to the qTest Parameters API
 * by making requests to various endpoints and checking the responses.
 * 
 * Usage:
 *   node test-qtest-parameters-connectivity.js --baseUrl=https://instance.qtestnet.com/api/v3 --token=YOUR_API_TOKEN
 * 
 * Options:
...
if (!values.baseUrl || !values.token) {
  ...
  console.log('Usage: node test-qtest-parameters-connectivity.js --baseUrl=https://instance.qtestnet.com/api/v3 --token=YOUR_API_TOKEN');
  process.exit(1);
}
...
async function runTests() {
  console.log(chalk.blue('🧪 Testing qTest Parameters API connectivity...'));
  console.log(chalk.gray(`Base URL: ${values.baseUrl}`));
  ...
  const parameters = await testEndpoint(
    'Get parameters', 
    'get', 
    `/parameters?projectId=${values.projectId}&page=1&size=10`
  );
  
  let parameterId = null;
  
  if (parameters && parameters.content && parameters.content.length > 0) {
    parameterId = parameters.content[0].id;
    ...
    await testEndpoint(
      ...
      'get', 
      `/parameters/${parameterId}/values?page=1&size=10`
    );
  } else {
    // Try to create a test parameter
    const newParameter = await testEndpoint(
      'Create parameter', 
      'post', 
      '/parameters', 
      {
        name: 'Test Browser',
        description: 'Browser type for testing',
        type: 'STRING',
        status: 'ACTIVE',
  ...
  const datasets = await testEndpoint(
    'Get datasets', 
    'get', 
    `/datasets?projectId=${values.projectId}&page=1&size=10`
  );
  
  let datasetId = null;
  
  if (datasets && datasets.content && datasets.content.length > 0) {
    datasetId = datasets.content[0].id;
    ...
    await testEndpoint(
      'Get dataset rows', 
      'get', 
      `/datasets/${datasetId}/rows?page=1&size=10`
    );
  } else if (parameterId) {
    // Try to create a test dataset
    const newDataset = await testEndpoint(
      'Create dataset', 
      'post', 
      '/datasets', 
      {
        name: 'Test Browser Dataset',
        description: 'Browser combinations for testing',
        status: 'ACTIVE',
        projectId: parseInt(values.projectId, 10),
        parameters: [parameterId]
      }
  ...
  if (failureCount > 0) {
    ...
    console.log();
    console.log(chalk.green('✅ All tests passed! qTest Parameters API is accessible and working.'));
  }
}
