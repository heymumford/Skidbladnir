...
function checkVersionConsistency() {
  ...
  if (buildVersion) {
    // For build-versions.json, we need to handle the -bXX suffix in a special way
    // Let's extract the base version without the build number
    const packageBase = packageVersion.replace(/-.*$/, '');
    const buildBase = buildVersion.replace(/-.*$/, '');
    
    if (packageBase !== buildBase) {
      logError(`Version base mismatch between package.json (${packageBase}) and build-versions.json (${buildBase})`);
  ...
  if (pyprojectVersion) {
    // For pyproject.toml, also handle potential -bXX or -test suffixes
    const packageBase = packageVersion.replace(/-.*$/, '');
    const pyprojectBase = pyprojectVersion.replace(/-.*$/, '');
    
    if (packageBase !== pyprojectBase) {
      logError(`Version base mismatch between package.json (${packageBase}) and pyproject.toml (${pyprojectBase})`);
      hasVersionMismatch = true;
    }
  }
